	  ...
	  $('.frame-option').click(function(){
		  console.log(this);
	  		$('.frame-option').each(function(i, d){
	  			 $('#'+(d.firstChild.dataset.href.split('#')[1])).css({display :'none'});
	  		})
	...
	$('#roomViewButton').click(function(){
		setTimeout(function(){
			...
			$('.wall-thumbnail>div>img').click(function() {
				...
				var img = $(this).clone();
				var photo = $('#image-window img#originalImage').clone();
	...
	frameData.forEach(function(d, i){
		...
		options.forEach(function(div, j){
			...
			if (d.split('-')[0].trim().toLowerCase() == div.id.split('_')[0]){
				...
				$(div).append("<div class=\"fram-thumbnail col-md-3 col-sm-3\" style=\"border-image-source: url(http://mithun.visionads.com.au/otw/public/web/photo_frames/"+d+")\"></div>");
			}
		})
...
function addModalIndex (selector, frameData) {
	...
	frameData.forEach(function(d, i){
		...
		divStr += "<div>";
		divStr += "<img src=\"http://mithun.visionads.com.au/otw/public/web/room_view/"+d+"\">";
		divStr += "</div>";
		divStr += "</div>";
	});
...
function setImage(photoHeight, id, flag){
	...
	if(flag){
		$(id+' img#imageString').outerWidth(width);
...
function matsWidth(oldHeight, newHeight, matsWidth, id, flag){
	...
	var matsWidth = +cmToPixel(matsWidth);
	...
	if(!flag){
		...
		var imgHeight = $(id+' img#originalImage').outerHeight(); 
		var imgWidth = $(id+' img#originalImage').outerWidth();
